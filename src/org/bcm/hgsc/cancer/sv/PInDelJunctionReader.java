...
public class PInDelJunctionReader implements JunctionSourceReader {
	private final BufferedReader reader;
	private JRecord next;
	private final Caller caller;
	
	public PInDelJunctionReader(String source, String params, int buffer, Caller caller) throws FileNotFoundException{
		this.caller = caller;
		this.reader = new BufferedReader(new FileReader(new File(source)));
		this.next = parseLine();
	}
	
	public PInDelJunctionReader(String source, Caller caller) throws FileNotFoundException{
		...
		try {
			...
			if (line == null){
				...
				if (lsplit.length < 12){
					...
					final int bases = Integer.parseInt(lsplit[2]);
					if (bases < AppendSVGraph.minSVSize){ 
						// not an indel by our standards
						return parseLine();
					}
					return new JRecord(
							lsplit[7], 
							Long.parseLong(lsplit[9]), 
							lsplit[7], 
							Long.parseLong(lsplit[10]), 
							Orientation.POS,
							Orientation.POS,
							Integer.parseInt(lsplit[4]), caller);
				}
			}
		} catch (IOException e) {
