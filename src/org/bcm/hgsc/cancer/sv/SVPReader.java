...
public class SVPReader implements JunctionSourceReader {

	private final BufferedReader reader;
	private JRecord next;
	private final Caller caller;
	
	public SVPReader(String source, String params, int buffer, Caller caller) throws FileNotFoundException{
		this.caller = caller;
		this.reader = new BufferedReader(new FileReader(new File(source)));
		this.next = parseLine();
	}
	
	public SVPReader(String source, Caller caller) throws FileNotFoundException{
	...
	private JRecord parseLine(){
		...
		try {
			...
			if (line == null){
				...
				if (lsplit.length < 10){
					System.err.println("Line did not have 10 fields");
